...
// with respect to them. Therefore, its parameters are not added to its
// name.
...
func (f bloomFilter) Contains(filter, key []byte) bool {
	...
	k := filter[nBytes] // cnt of hash functions...
	if k > 30 {
...
func (f bloomFilter) NewGenerator() FilterGenerator {
	...
	// k here, cnt of hash functions...
	k := uint8(f * 69 / 100) // 0.69 =~ ln(2), f is bits per key.
	if k < 1 {
		k = 1
	} else if k > 30 {
		k = 30
...
func (g *bloomFilterGenerator) Generate(b Buffer) {
	// len(g.keyHashes) as n, nBits as m.
	...
	dest[nBytes] = g.k // k hash functions... encoded as last byte...
